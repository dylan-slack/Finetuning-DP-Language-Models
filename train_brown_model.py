...
if __name__ == "__main__":
	...
	model_state = load_model(LM, optimizer, "brown_pretrained_big/epoch_2_it_500.tar", False)
	...
	for E in range(1, 20):
		...
		for x_batch, y_batch in tqdm(data_generator(X_train, Y_train, batch_size), desc="Iterating over epoch {}".format(E + 1)):
			...
			# 	save_checkpoint(**best_model, dir_name="brown_pretrained_big", file_name="best_model.tar")
			...
			if it % 500 == 0:
				...
				# save checkpoint
				save_checkpoint(E + 1, it, LM.state_dict(), optimizer.state_dict(), l, "brown_pretrained_big", "epoch_{}_it_{}.tar".format(E + 1, it))
